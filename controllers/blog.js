...
exports.create = (req, res) => {
  ...
  form.parse(req, (err, fields, files) => {
    ...
    if (!body ||!body.length|| body.length < 200) {
      return res.status(400).json({
    ...
    blog.mdesc = stripHtml(blog.excerpt.substr(0, 160)).result;
...
exports.list=(req,res)=>{
  ...
  .select('_id title slug excerpt categories tags hasphoto postedBy createdAt updatedAt')
  .exec((err,data)=>{
    if(err){
      return res.json({
        error:errorHandler(err)
...
//listing blogs with categories and tags
...
exports.read=(req,res)=>{
    ...
    .populate('tags','_id name slug')
    .populate('postedBy','_id name username')
    .select('_id title body slug mtitle mdesc categories hasphoto tags postedBy createdAt updatedAt')
     .exec((err,data)=>{
      if(err){
        return res.json({
          error:errorHandler(err)
